...
/**
 * AiFddReaderCaptureForm: email capture form for the AI Paperwork Reader.
 * Posts to /api/ai-fdd-reader (name + email). Mirrors EscapeKitCaptureForm.
 */
export default function AiFddReaderCaptureForm({
  articleSlug = "",
}: {
  articleSlug?: string;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/ai-fdd-reader", {
        method: "POST",
    ...
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-[#5a5a4a] leading-relaxed">
          Check your inbox. The prompt pack is on its way. If you have questions,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3" id="ai-fdd-reader-form">
      <div className="flex flex-col md:flex-row gap-3">
        <label htmlFor="ai-fdd-reader-name" className="sr-only">First name</label>
        <input
          type="text"
          id="ai-fdd-reader-name"
          name="name"
        ...
        />
        <label htmlFor="ai-fdd-reader-email" className="sr-only">Your email address</label>
        <input
          type="email"
          id="ai-fdd-reader-email"
          name="email"
          autoComplete="email"
