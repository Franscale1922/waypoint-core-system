...
/**
 * PitchDecoderCaptureForm: email capture form for the Franchise Pitch Decoder.
 * Posts to /api/pitch-decoder (name + email). Mirrors EscapeKitCaptureForm.
 */
export default function PitchDecoderCaptureForm({
  articleSlug = "",
}: {
  articleSlug?: string;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/pitch-decoder", {
        method: "POST",
    ...
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-[#5a5a4a] leading-relaxed">
          Check your inbox. The Pitch Decoder is on its way. If you have questions,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3" id="pitch-decoder-form">
      <div className="flex flex-col md:flex-row gap-3">
        <label htmlFor="pitch-decoder-name" className="sr-only">First name</label>
        <input
          type="text"
          id="pitch-decoder-name"
          name="name"
        ...
        />
        <label htmlFor="pitch-decoder-email" className="sr-only">Your email address</label>
        <input
          type="email"
          id="pitch-decoder-email"
          name="email"
          autoComplete="email"
        ...
        >
          {status === "loading" ? "Sending…" : "Send me the Pitch Decoder"}
        </button>
      </div>
      {status === "error" && (
