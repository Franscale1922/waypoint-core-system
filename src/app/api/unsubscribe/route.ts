...
export async function GET(req: NextRequest) {
  ...
  if (!id || !token) {
    return new NextResponse(unsubscribePage("Invalid unsubscribe link.", true), {
      status: 400,
  ...
  if (!verifyUnsubscribeToken(id, token)) {
    return new NextResponse(unsubscribePage("This unsubscribe link is invalid or has expired.", true), {
      status: 400,
      headers: { "Content-Type": "text/html" },
    });
  }
  ...
  return new NextResponse(
    unsubscribePage("You've been unsubscribed. You won't receive any more notes from this sequence.", false),
    { status: 200, headers: { "Content-Type": "text/html" } }
  );
}

function unsubscribePage(message: string, isError: boolean): string {
