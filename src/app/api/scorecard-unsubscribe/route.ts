...
export async function GET(req: NextRequest) {
  ...
  const record = await (prisma as any).scorecardSubmission.findUnique({ where: { id } });
  if (!record) {
    return new NextResponse(buildPage("We couldn't find that subscription. You may have already been removed.", false), {
      status: 200,
      headers: { "Content-Type": "text/html" },
    });
  }
