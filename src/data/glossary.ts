...
// `faqs` is optional and deliberately sparse. Every term page already answers
// "what is <term> in franchising?" from the definition, which is enough for most
// entries. Add FAQs only where Search Console shows people asking the term
// several distinct ways, so the extra FAQPage entries mirror real queries
// instead of inventing questions nobody typed.
export type GlossaryFaq = { q: string; a: string };
export type GlossaryEntry = { term: string; definition: string; related?: string | null; relatedLabel?: string | null; faqs?: GlossaryFaq[] };
export type GlossaryGroup = { letter: string; entries: GlossaryEntry[] };

export const terms: GlossaryGroup[] = [
  {
    letter: "A",
    entries: [
      {
      ...
      },
      {
        ...
        related: "/resources/fdd-decoded-what-actually-matters",
        relatedLabel: "FDD decoded: what actually matters",
        // The AUV cluster is the single largest source of impressions to this
        // site: 69 across eight distinct queries in the 28 days to 2026-08-01,
        // against a page sitting at position 37. People ask it three different
        // ways ("auv meaning", "what does auv mean", "average unit volume"), and
        // the page answered only one of them.
        faqs: [
          {
            q: "What does AUV mean?",
            a: "AUV stands for average unit volume. It is the average annual revenue of a single franchise location, found by dividing total system-wide sales by the number of open units. It describes one typical location rather than the whole system.",
          },
          {
            q: "How is average unit volume calculated?",
            a: "Divide the system's total annual sales by the number of units open for the full period. Franchisors that publish financial performance data include it in the franchise disclosure document you receive before signing. Read the footnotes closely, because some brands report only their top-performing units or exclude locations open less than a year, which raises the figure.",
          },
          {
            q: "Is AUV the same as profit?",
            a: "No. AUV measures gross sales before any expenses. Two locations with the same AUV can leave their owners in very different positions once rent, labor, royalties, and supply costs come out. Treat AUV as the top line and ask separately what the cost structure looks like.",
          },
          {
            q: "What does AUV mean in a restaurant?",
            a: "The same thing it means anywhere else in franchising: the average annual sales of one location. The term simply comes up more in restaurants, because sales volume per location is the number the industry leads with. A restaurant brand quoting its average unit volume is describing what a typical location rings up in a year, not what the owner keeps after food costs, labor, rent, and royalties.",
          },
          {
            q: "What is a good average unit volume?",
            a: "There is no universal threshold, and a number that looks strong in one category can be weak in another. A quick-service restaurant and a home-based service business have completely different cost structures, so comparing their volumes tells you very little. Compare a brand only against others in its own category, and pair the figure with what it costs to open and operate that location. A high volume carried by high overhead is not automatically better than a lower one with light costs.",
          },
        ],
      },
    ],
