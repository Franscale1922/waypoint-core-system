...
| | |
|---|---|
| `main` | `aaa8437` — PRs #22, #24, #25 reviewed, merged, and deployed 2026-08-04 |
| `seo/investment-selection-intent` | **merged as #25** (`24530c1`), branch deleted |
| `seo/auv-cluster` | **merged as #24** (`c4dd163`), branch deleted |
| `seo/structured-data-entity-graph` | **merged as #22** (`aaa8437`) — this row said "pushed, no PR" until 2026-08-04 |
| `claude/beautiful-napier-ede2fc` | `0614c7c` — **PR #26 open**. Its base branch has now merged, so #26 is unblocked; see below |
| `claude/competent-easley-9eec18` | `fa7524b` — **PR #23 open**, aeo-audit gate hardening, base `main`. Merging is a go-live |
...
**Superseded 2026-08-04: the base branch merged as PR #22 (`aaa8437`).** The stack is gone and #26
is unblocked. The original reason for targeting the base rather than `main` (keeping #26's diff to
just the VideoObject work) is now satisfied by `main` itself, so **retarget #26 at `main`** — it no
longer drags in five unrelated commits, because those are what merged. The instruction below is kept
for the reasoning, not the sequence.

~~Nothing merges until the base branch gets a PR. PR #26 deliberately targets the base rather than
`main` so its diff is only the VideoObject work; retargeting it at `main` would drag in five
