...
- **2026-06-18 — Task 8 [RESOLVED on `main`]:** The original sweep was discarded in the reset, but it was then **re-run on `main`**: 537 em dashes removed across 89 files in `src/app` + `src/data` (commit `25a4cb5`), and `aeo-audit.mjs` was extended to walk `src/app`/`src/data` and **exit non-zero on any em dash** (commit `4ddbc34`), so §11 is now enforceable in CI. Verified: guard passes (exit 0), `grep` count 0 site-wide, typecheck clean, `npm test` green, entity-graph links preserved. §11 is now clean across articles, `src/app`, and `src/data`.
