...
- **2026-06-18 — Task 8 DONE:** Sitewide em-dash sweep (§11). Removed all 367 em dashes across 71 files (43 `.tsx` + 28 `.ts`/email/CSS; §11 covers email + agent copy). Verified: 0 em dashes in `src/app` + `content/articles`; typecheck = only the 6 pre-existing Prisma errors, zero new. 5 minor judgment spots flagged for review (admin empty-cell glyphs → en dash, email sign-offs de-dashed). Open follow-on: extend `aeo-audit.mjs` to scan `src/app` (not just markdown) so this can't regress.
...
**STATUS: ALL 8 tasks complete + Phase-1 schema. Nothing committed — awaiting (1) verification of the investment-by-category dollar ranges, (2) go-ahead to stage commits.**
